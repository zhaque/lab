...
# About Unicode & UTF-8: 
# All data is Unicode. When we need something in UTF-8, we encode it.
#   The only exception now is _query, which is UTF-8. Sorry!
# For example, these needs UTF-8: 
#  quote(), quote_plus(), 
#  urlencode() unless you use doseq=1 param
#  hashlib hexdigest()
...
class Service(Thread):
    ...
    def __init__(self, query, mashup, service_params, start=0, count=3):
        """
...
class HttpCachableService(CachableService):
    ...
    def process_result(self, result):
        ...
        # Generate the hash of it (it must be a string for hash to work)
        if type(raw_id) is int:
            raw_id = "%s" % raw_id
        id = hashlib.new(raw_id.encode('utf-8')).hexdigest()
...
class SolrTwitterService(SolrService):
    """Runs a solr query for tweets.
    
    How we build the solr query (all of this code is in get_params below):
    If this is a recursive query to build the conversation:
      id:<in_reply_to_status_id>
      -- and no more params --

    Else, for all normal queries:
        If sort by date (front end param):
          q={!lucene q.op=AND}text:<frontend query>
          sort=created_at_dt desc
        Else, if sort by relevance:
          q={!lucene q.op=AND}text:<frontend query> _val_:" product(sum(scale(created_at_dt,0,1), scale(author_followers_count_i, 0, 1)), 0.5)"
          (no sort param to solr)
          Note: if you want to change the _val_ param without overriding 
          the whole q= param, you have to modify the code below (this could be in the backend admin separately if you need)
          
        If date_from or date_to (frontend params):
          fq=created_at_dt:[<from> TO <to>]  (if one of these is not specified,  *  will be used)
        
        And finally, from the SolrHandlerConfig that you configured in the
        admin backend (as described in /README), override all fields.
        So if in the admin backend you specify q=tata, you are overridding
        the entire q= param created before.
    ...
    def get_params(self):
        "Return GET params for the solr query url (See long comment above)"
        
        params = super(SolrTwitterService, self).get_params();
        
        # If this is process_result() that is recursively tracking the conversation, 
        # search by status id
        if 'in_reply_to_status_id_i' in self._service_params:
            params['q'] = "id:%s" % self._service_params['in_reply_to_status_id_i']
            # That's it; no more params for this query
        
        else:
            
            if self._service_params['sort_by'] == "Relevance":
                valstr = ' _val_:" product(sum(scale(created_at_dt,0,1), scale(author_followers_count_i, 0, 1)), 0.5)"'
                sort = ''
            else:
                valstr = ''
                sort = 'created_at_dt desc'
                    
            q = '{!lucene q.op=AND}%s%s%s' % (self._field, self._query, valstr)
            
            fq = ""
            from_ = self._service_params.get('date_from', "*") or "*"
            to_ = self._service_params.get('date_to', "*") or "*"
            if from_ != "*" or to_ != "*":
                fq = "created_at_dt:[%s TO %s]" % (from_, to_)
            ...
            # Add params from the admin
            for f in self._mashup.solr_handler_config.solrfield_set.all():
                params[f.field_name] = f.field_value
    ...
    def process_result(self, doc):
        ...
        #
        # Build conversation backwards in time. Leave resulting list in 'conversation'
        # If the status referenced by
        # in_reply_to_status_id doesn't exist in solr, the conversation is cut there.
        #
        if 'in_reply_to_status_id_i' in doc  and \
        self._service_params.get('depth', 0) < settings.MAX_TWITTER_CONVERSATION_DEPTH:
            # Search by status id
            service_params = {
                              'in_reply_to_status_id_i': doc['in_reply_to_status_id_i'],
                              'depth': self._service_params.get('depth', 0) + 1
                              }
            service = SolrTwitterService("", self._mashup, service_params, start=0, count=3) # TODO:
            service.run()
            
            # Will contain a [] of fully processed twitter results (with voting info, etc.).
            # The list will be ordered from oldest to most recent
            conversation = []
            results = service.results()
            if len(results) >= 1:  # There should be just 1 or none with that ID
                r = results[0]
...
class TwitterTrendsService(HttpCachableService):
    ...
    def process_result(self, result):
        # DO NOT call process_result on super class; we don't need voting
        # and don't need AddToAny
